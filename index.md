---
layout: home
bg: "bg-taupe-50"
title: "Full-Stack Developer & Designer in Knoxville"
---

## Hello there, my name is Josh. I’m a full-stack developer, engineer, and designer in <span class="vols text-rocky-top">Knoxville</span>, Tennessee I have over a decade of experience building highly performant websites for multiple disciplines.
{: .lead-paragraph}

#### Lately, I've been building
{: .not-prose .mt-10 .mb-4 .text-sm .font-semibold .uppercase .tracking-wide .text-taupe-500 }

- [**Beaconed**](https://beaconed.ai) — an app that gets online stores ready for AI shopping agents, auto-generating the alt text, descriptions, and structured data those agents read.
- [**Clementine**](https://clementinecsa.com) — a CSA platform for small farms: a members list, weekly recurring card charges, and a shareable signup page, all in one place.
- [**Dinner Letter**](https://dinnerletter.com) — a weekly meal-planning service that builds a personalized dinner plan around each household's tastes and schedule. A project of Sola Co.
- [**Saints Church**](https://saintschurchknox.com) — a fast little Jekyll site for my church here in Knoxville.

I also head up engineering at [**AltText.AI**](https://alttext.ai), where we make the web more accessible with AI-generated image descriptions.

<a href="{{site.url}}/resume" class="not-prose jre-button">Read My Resume</a>
